...
export async function POST(req: Request) {
  try {
    ...
    // Set the JWT as a cookie (temporarily removed HttpOnly for debugging)
    const headers = new Headers();
    headers.append(
      'Set-Cookie',
      `token=${token}; Path=/; Max-Age=7200; SameSite=Strict` // Remove HttpOnly temporarily for testing
