...
export default function TealCertificatesPage() {
  ...
  const categories = [
    "all",
    ...Array.from(new Set(languages.map((lang) => lang.category))),
  ];
      ...
      <div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <input
            ...
            } border ${isDarkMode ? "border-teal-700" : "border-teal-300"}`}
          />
        </div>
        ...
        >
          {filteredLanguages.map((language) => {
            const IconComponent = LucideIcons[language.icon] as LucideIcon;
            return (
              <Link
                href={`/certificates/${language.name
                  .toLowerCase()
                  .replace(/\s+/g, "-")}`}
                key={language.name}
              >
                  ...
                  >
                    ...
                    {viewMode === "list" && (
                      <div className="flex items-center space-x-4 w-2/3">
                        <span
                          className={`inline-block px-2 py-1 rounded-full text-xs ${
                            difficultyColors[language.difficulty]
                          } capitalize`}
                        >
                          {language.difficulty}
                        </span>
