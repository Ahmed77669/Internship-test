...
const Exams = () => {
  ...
  const [isMounted, setIsMounted] = useState(false); // Added to check if component is mounted
  const currentQuestion = questions[questionIndex];
  const typingSpeed = 10;

  useEffect(() => {
    setIsMounted(true); // Mark as mounted when component is rendered on the client
  ...
  useEffect(() => {
    if (!isMounted || !router.query.quiz) return; // Avoid using router until component is mounted

    async function fetchData() {
      try {
        const response = await fetch(
          `https://exam.gammal.tech/API/quiz/${router.query.quiz}`,
          {
            method: "GET",
            headers: {
    ...
    if (currentQuestion && currentQuestion.options) {
      ...
      if (index < currentQuestion.question.length) {
        const timeout = setTimeout(() => {
          setDisplayedText(
            (prev) => prev + currentQuestion.question.charAt(index)
          );
        ...
        <div className="flex items-center">
          ...
          <p
            className={`ml-2 text-xl md:text-2xl ${
              time < 11 ? "text-red-500" : "text-black"
            }`}
          >
            {time}
          </p>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start p-2 md:p-8">
        <div className="w-full lg:w-2/3 h-[300px] md:h-[500px] lg:h-[700px] bg-black mb-6 lg:mb-0 rounded-lg p-4 md:p-8 overflow-auto">
          <pre className="text-green-400 text-sm md:text-base">
            {quizzes.length > 0 ? quizzes[0].question_text : "Loading..."}
          </pre>
        </div>
