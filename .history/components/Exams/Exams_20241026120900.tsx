...
import questions from "@/app/dummy-data/questions"; // This seems unused in your code
...
interface Quiz {
  ...
  code: string;
  Choices: { choice_id: number; question_id: number; choice_text: string; is_correct: boolean }[]; // Updated to match the API structure
}
...
const Exams: React.FC<ExamsProps> = ({ quiz }) =>{
  ...
  // Ensure the current question is fetched correctly
  ...
  useEffect(() => {
    if (!quiz) return; // Wait for quiz to be available
  ...
  useEffect(() => {
    if (!currentQuestion) return; // Wait for current question to be available
    if (prevQuestion !== currentQuestion.code) {
      setDisplayedText("");
      setIndex(0);
      setPrevQuestion(currentQuestion.code); // Corrected to use `currentQuestion.code`
    }

    if (index < currentQuestion.code.length) { // Use code length instead of question length
  ...
  const handleAnswerClick = (option: string) => { // Changed to use option parameter
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start p-2 md:p-8">
        <div className="w-full lg:w-2/3 h-[300px] md:h-[500px] lg:h-[700px] bg-black mb-6 lg:mb-0 rounded-lg p-4 md:p-8 overflow-auto">
          <pre className="text-green-400 text-sm md:text-base">
            {quizzes.length > 0 ? quizzes[questionIndex].code : "Loading..."}
          </pre>
        </div>
        <div className="w-full lg:w-1/3 flex flex-col items-center justify-center h-[70vh]">
          <div className="border border-black rounded-xl p-4 w-full max-w-[400px]">
            <div className="text-center text-xl md:text-2xl text-[#006565] mt-4">
              Question {quizzes[questionIndex].question_id}
            </div>
            <div className="text-left text-base md:text-lg text-black mt-6 ml-2">
            {quizzes[questionIndex].question_text}
            </div>
            <div className="flex flex-col items-center justify-around mt-4">
              {currentQuestion.Choices.map((choice) => (
                <button
                  ...
                  onClick={() => handleAnswerClick(choice.choice_text)} // Updated to pass choice_text
                >
                  {choice.choice_text}
                </button>
              ))}
