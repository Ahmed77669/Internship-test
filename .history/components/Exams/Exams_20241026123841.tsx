...
const Exams: React.FC<ExamsProps> = ({ quiz }) =>{
  ...
  useEffect(() => {
    // Check if `currentQuestion` and its `code` are loaded before starting typing animation
    if (!currentQuestion || !currentQuestion.code) return;
  
    // Reset the displayed text and index when moving to a new question
    if (prevQuestion !== currentQuestion.code) {
      setDisplayedText(""); // Clear the displayed text
      setIndex(0);           // Reset typing index
      setPrevQuestion(currentQuestion.code); // Update previous question to current one
    }
  
    // Start typing out `currentQuestion.code` character by character
    if (index < currentQuestion.code.length) {
      const timeout = setTimeout(() => {
        // Append the next character to `displayedText`
  ...
  }, [index, currentQuestion, prevQuestion, typingSpeed]); // Add `typingSpeed` and `currentQuestion` as dependencies
  useEffect(() => {
