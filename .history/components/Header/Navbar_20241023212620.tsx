...
export default function NavBarHome() {
  ...
  // Check for the token on mount to determine if the user is authenticated
  useEffect(() => {
        ...
        <ul className="flex justify-between items-center">
          ...
          {!isAuthenticated ? (
            <div className="hidden md:flex space-x-5">
              ...
              <Link href="/about" className="text-[#007676] text-[18px] font-normal flex items-center">Contact Us</Link>
              <Link href="/signup">
            ...
            <div className="hidden md:flex space-x-5">
              <Link href="/dashboard" className="text-[#007676] text-[18px] font-normal flex items-center">Dashboard</Link>
        ...
        {isOpen && (
          <div className="absolute right-0 mt-2 w-full bg-white shadow-lg rounded-lg z-20 transition-transform transform origin-top-right scale-100">
            <div className="flex flex-col items-center justify-center mx-auto p-4">
              <Link href="/" className="text-[#007676] text-lg font-semibold hover:bg-[#f0f0f0] transition duration-200 p-3 rounded-lg text-center w-full" onClick={() => setIsOpen(false)}>
                Home
              </Link>
              <Link href="/pricing" className="text-[#007676] text-lg font-semibold hover:bg-[#f0f0f0] transition duration-200 p-3 rounded-lg text-center w-full" onClick={() => setIsOpen(false)}>
                Pricing
              </Link>
              <Link href="/about" className="text-[#007676] text-lg font-semibold hover:bg-[#f0f0f0] transition duration-200 p-3 rounded-lg text-center w-full" onClick={() => setIsOpen(false)}>
                About Us
              </Link>
              <button className="mt-4 bg-[#007676] text-white text-lg font-semibold py-2 px-6 rounded-lg hover:bg-[#005c5c] transition duration-200" onClick={() => setIsOpen(false)}>
                GET STARTED
              </button>
            </div>
          </div>
