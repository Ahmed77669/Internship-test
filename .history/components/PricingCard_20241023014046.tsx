...
const PricingCard: React.FC<PricingCardProps> = ({ plan, price, attempts, features, isPopular }) => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      setLoading(false); // Set loading to false when script is ready
    };

    script.onerror = () => {
      console.error('Failed to load FawryPay script.');
      setLoading(false); // Set loading to false even if the script fails
    };
  ...
  return (
    <div className={`w-full max-w-sm p-6 bg-white border rounded-lg shadow-md transition-all duration-300 ${isPopular ? 'border-teal-500 shadow-lg' : 'border-teal-200 hover:border-teal-300'}`}>
      ...
      <button
        ...
        disabled={loading || !isFawryPayReady} // Disable the button while loading or if FawryPay is not ready
      ...
      >
        {loading ? 'Loading...' : 'Get Started'} {/* Change button text based on loading state */}
      </button>
    </div>
  );
};
